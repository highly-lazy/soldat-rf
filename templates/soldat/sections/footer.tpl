<footer class="footer">
	<div class="container footer__inner">
		<a class="footer__logo" href="{home}/" aria-label="солдат.рф — на главную">
			<img src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-lazy-src="{theme}/images/e11e938b761fbff026f2e1fc125893e81aef3faa.svg" alt="солдат.рф" width="173" height="30" decoding="async">
		</a>

		<p class="footer__copy">
			2026 © Солдат.рф Все права защищены. Не является публичной офертой.
			<a href="https://xn--80ahulok.xn--p1ai/policy.html" target="_blank" rel="noopener">Политика конфиденциальности</a>
		</p>

		<ul class="footer__messengers">
			<li class="footer__messenger footer__messenger--tg">
				<a href="https://t.me/" aria-label="Telegram"><img src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-lazy-src="{theme}/images/080db21598596027d30d9f5af98ca8efbbaa1c3b.svg" alt="" width="29" height="24" decoding="async"></a>
			</li>
			<li class="footer__messenger footer__messenger--max">
				<a href="https://max.ru/" aria-label="MAX">
					<img src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-lazy-src="{theme}/images/ca7b5356fc8cf41cc282f4259585dcc7e5aa9954.svg" alt="" width="27" height="27" decoding="async">
					<img class="footer__cursor" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-lazy-src="{theme}/images/fbbff32a6e52ff29305d58cb0127bf5cf5925761.svg" alt="" width="17" height="22" decoding="async">
				</a>
			</li>
			<li class="footer__messenger footer__messenger--wa">
				<a href="https://wa.me/74954450117" aria-label="WhatsApp"><img src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-lazy-src="{theme}/images/734d4551fa9e221288b2aee887b03459d8d35482.svg" alt="" width="30" height="30" decoding="async"></a>
			</li>
		</ul>
	</div>
</footer>
