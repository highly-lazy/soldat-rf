<section class="cta cta--article" aria-labelledby="cta-title">
	<div class="container">
		<div class="cta__panel">
			<img class="cta__photo" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-lazy-src="{theme}/images/cta-article.webp" alt="" width="1401" height="394" decoding="async" loading="lazy">

			<div class="cta__body">
				<h2 class="cta__title" id="cta-title">Остались вопросы?</h2>
				<p class="cta__lead">Оставь заявку — и наши специалисты помогут тебе пройти все этапы оформления контракта, ответят на вопросы и подберут лучшие условия службы.</p>
			</div>

			<ul class="cta__messengers">
				<li class="cta__messenger cta__messenger--tg">
					<a href="https://t.me/" aria-label="Telegram"><span class="cta__icon" aria-hidden="true"></span></a>
				</li>
				<li class="cta__messenger cta__messenger--max">
					<a href="https://max.ru/" aria-label="MAX"><span class="cta__icon" aria-hidden="true"></span></a>
				</li>
				<li class="cta__messenger cta__messenger--wa">
					<a href="https://wa.me/74954450117" aria-label="WhatsApp"><span class="cta__icon" aria-hidden="true"></span></a>
				</li>
			</ul>
		</div>
	</div>
</section>
